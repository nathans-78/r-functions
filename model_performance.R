...
model_performance <- function(model, predict, actual) {
  ...
  results_consolidated <- results[, .(model, accuracy=accuracy, precision, weighted_precision=precision*weight, recall, weighted_recall=recall*weight, f1_score, weighted_f1_score=f1_score*weight)]
  results_consolidated <- results_consolidated[, .(accuracy=mean(accuracy), mean_precision=mean(precision), weighted_precision=sum(weighted_precision), mean_recall=mean(recall), weighted_recall=sum(weighted_recall), mean_f1_score=mean(f1_score), weighted_f1_score=sum(weighted_f1_score)), by=.(model)]
  return(list("confusion_matrix"=confusion_matrix, "performance_results"=results, "results_consolidated"=results_consolidated))
}
